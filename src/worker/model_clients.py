...
def fake_translate(contexto: Dict[str, Any]) -> Dict[str, Any]:
    """
    Simula a resposta de um modelo de linguagem para viabilizar a demonstração
    distribuída no AWS Academy, sem usar Bedrock, serviço externo ou API externa.
    """
    inicio = time.time()
    ...
    # Pequena latência artificial para as métricas fazerem sentido na apresentação.
...
def call_model_once(contexto: Dict[str, Any]) -> Dict[str, Any]:
    """
    Nesta versão do trabalho, apenas o modo fake é usado.
    Isso evita dependência de GPU ou serviços externos.
    """
    return fake_translate(contexto)
